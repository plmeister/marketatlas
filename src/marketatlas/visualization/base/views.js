...
class ChartView {
  ...
  _clearPriceLines() {
    ...
    if (this.candleSeries) {
      ...
      this.tradePriceLines.forEach((pl) =>
  ...
  updateATR(idx) {
    ...
    this.atrSeries.setData(
      this.model.atrData.filter((d) => d.time <= frameTime),
    );
  }
    ...
    this.model.activeTradesAt(frameTime).forEach((t) => {
      ...
      this.tradePriceLines.push(
        this.candleSeries.createPriceLine({
          ...
          lineStyle: LightweightCharts.LineStyle.Dashed,
          axisLabelVisible: true,
          title: "Target " + t.target.toFixed(2),
        }),
      );
    });
  }
  ...
  updateZigzag(idx) {
    ...
    Object.values(facts).forEach((val) => {
      ...
      const series =
        val.direction === "bullish" ? this.zigzagBull : this.zigzagBear;
  ...
  resize() {
    ...
    if (this.atrChart)
      this.atrChart.applyOptions({ width: this.containers.atr.clientWidth });
    if (this.volumeChart)
...
class InfoPanelView {
  ...
  update(model, idx) {
    ...
    const remaining = model.activeCandles.filter(
      (c) => c.time > frameTime,
    ).length;
    html +=
    ...
    Object.entries(facts).forEach(([key, val]) => {
      if (val.type === "ema") {
        ...
        const cls =
          val.direction === "bullish"
            ? "bull"
            : val.direction === "bearish"
              ? "bear"
              : "neutral";
        html +=
    ...
    // S/R
    const srLevels = model
      .srLevelsAt(idx)
      .filter((lv) => lv.strength >= model.minTouches);
    if (srLevels.length) {
...
class EvidencePanelView {
  ...
  update(model, idx) {
    ...
    if (!evidence.length) {
      this.el.innerHTML =
        idx < model.frames.length
          ? "No evidence for this frame."
          : "No evidence.";
      return;
    }
...
class SummaryBarView {
  update(model, idx) {
    ...
    document.getElementById("s-drawdown").textContent =
...
class TimelineBarView {
  ...
  build(model) {
    ...
    model.trades.forEach((t, i) => {
      ...
      const endPct =
        t.exit_time !== null
          ? (((_t(t.exit_time) - minTime) / span) * 100).toFixed(2)
          : "100";
  ...
  updateCursor(model, idx) {
    ...
    const maxTime = _t(
      model.activeCandles[model.activeCandles.length - 1].time,
    );
    const span = maxTime - minTime || 1;
    cursor.style.left =
      (((_t(frameTime) - minTime) / span) * 100).toFixed(2) + "%";
  }
}
