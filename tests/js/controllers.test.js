...
const { makeModel } = require("./sample_data.js");
const {
  PlaybackController,
  KeyboardController,
  TFController,
...
test("jumpToNextEvent advances frame by frame through events", () => {
  const model = makeModel();
  const pb = new PlaybackController(model, {}, { speed: 100, render: () => {} });
  model.goTo(0);
...
test("jumpToPrevEvent moves back through events", () => {
  const model = makeModel();
  const pb = new PlaybackController(model, {}, { speed: 100, render: () => {} });
  model.goTo(model.frames.length - 1);
  pb.jumpToPrevEvent(); // frame 4 (trade 3 entry)
  assert.strictEqual(model.currentFrame, 4);
  pb.jumpToPrevEvent(); // frame 3 (trade 2 entry)
  assert.strictEqual(model.currentFrame, 3);
  model.goFirst();
  pb.jumpToPrevEvent(); // no earlier events — stays put
  assert.strictEqual(model.currentFrame, 0);
});
...
test("n and p keys jump between significant events", () => {
  const model = makeModel();
  const pb = new PlaybackController(model, {}, { speed: 100, render: () => {} });
  const kb = new KeyboardController(pb);
...
function makeSpyViews() {
  ...
  chart.getVisibleTimeRange = () => {
    calls.getVisibleTimeRange.push("call");
    return null;
  };
  chart.setVisibleTimeRange = (r) => {
    calls.setVisibleTimeRange.push(r);
  };
  const panels = {};
...
function makeBackend(model) {
  const backend = {
    ...
    rebuildCalls: [],
    loadTFData(tf) {
      this.loadCalls.push(tf);
      return model.candlesByTF[tf] !== undefined;
    },
    rebuildCharts(tf) {
      this.rebuildCalls.push(tf);
    },
  };
  return backend;
}
