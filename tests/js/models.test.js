...
function assertClose(actual, expected, tolerance, msg) {
  ...
  if (Math.abs(actual - expected) > tolerance) {
    throw new Error(`${msg || ''} expected ${expected} ≈ ${actual}, diff ${Math.abs(actual - expected)} > ${tolerance}`);
  }
}
...
test('constructs with all data', () => {
  model = new AppModel({
    CANDLES, CANDLES_BY_TF, FRAMES, EMA_SERIES, ATR_DATA,
    SR_DATA, TRADES, PULLBACKS, FACTS_DATA, EVIDENCE_MAP,
    INITIAL_BALANCE: 10000, MIN_TOUCHES: 2,
  });
